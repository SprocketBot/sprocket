  ...
  private async performCrossFranchiseValidation(
    ...
    if (existingFranchises.has(eligibleFranchise.id)) {
      return {
        ...
        context: {
          existingRatifiers: this.getExistingRatifiersForFranchise(
            submission,
            eligibleFranchise.id,
          ),
...
class RatificationDataMigrationService {
    ...
    for (const submission of submissions) {
      try {
        ...
        if (migrationResult.status === 'success') {
          ...
          results.failed++;
          this.logger.error(
            `Failed to migrate submission ${submission.id}:`,
  ...
  private async migrateSubmission(
    submission: ReplaySubmission,
  ): Promise<SubmissionMigrationResult> {
    ...
    for (const playerId of submission.ratifiers) {
      try {
        ...
        this.logger.warn(
          `Could not determine franchise for player ${playerId} in submission ${submission.id}`,
        );
