...
export const Cache: (co: CacheOptions) => MethodDecorator = (
  ...
  function (target: Object, propertyKey: string | symbol, descriptor: PropertyDescriptor) {
    // Only for display purposes; when using verbose mode
    const ratio = {
      total: 0,
      hits: 0,
      misses: 0,
      uniques: {} as Record<string, Set<string>>,
    };
    ...
    if (verbose)
      logger.verbose(
        `Caching has been attached to ${subject}. Keyed Params: ${JSON.stringify(
          cacheKeys.map(i => paramNames[i]),
        )}`,
      );
