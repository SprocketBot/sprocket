...
export class RmqService {
  ...
  // Exchange key for the events "system"
  private readonly exchangeKey = config.transport.events_queue;
  ...
  async onApplicationBootstrap(): Promise<void> {
    const rmqUrl = config.transport.url;
  ...
  async sub(topic: string, instanceExclusive: boolean): Promise<Observable<ConsumeMessage>> {
     ...
     */
    let queue = `${this.exchangeKey}-${topic}-${config.transport.events_application_key}`;
