...
export class EventProxyService implements Omit<EventsService, "rmqService" | "subscribeAsyncIterator"> {
    constructor(private readonly service: EventsService, private readonly scrimMetricsService: ScrimMetricsService) {}

    async publish<T extends EventTopic>(topic: T, payload: EventPayload<T>, subtopic?: string): Promise<boolean> {
        await this.service.publish(EventTopic.ScrimMetricsUpdate, await this.scrimMetricsService.getScrimMetrics());
