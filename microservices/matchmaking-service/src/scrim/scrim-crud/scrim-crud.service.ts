...
export class ScrimCrudService {
  ...
  async getAllScrims(skillGroupId?: number): Promise<Scrim[]> {
    ...
    const scrims = await Promise.all(
      scrimKeys.map<Promise<Scrim>>(async key =>
        this.redisService.getJson<Scrim>(key, undefined, ScrimSchema),
      ),
    );
  ...
  async getScrimByPlayer(id: number): Promise<Scrim | null> {
    ...
    for (const key of scrimKeys) {
      const playerIds = await this.redisService.getJson<number[] | null | undefined>(
        key,
        '$.players[*].id',
      );
      ...
      if (playerIds.includes(id))
        return this.redisService.getJson<[Scrim]>(key, '$').then(([r]) => r);
    }
    return null;
  }
