...
export class ScrimCrudService {
    ...
    async getAllScrims(skillGroupId?: number): Promise<Scrim[]> {
        ...
        const scrims = await Promise.all(
            scrimKeys.map<Promise<Scrim>>(async key =>
                this.redisService.getJson<Scrim>(key),
            ),
        );
    ...
    async addPlayerToScrim(
        scrimId: string,
        player: ScrimPlayer,
    ): Promise<void> {
        await this.redisService.appendToJsonArray<ScrimPlayer>(
            `${this.prefix}${scrimId}`,
            "players",
            player,
        );
    ...
    async removePlayerFromScrim(
        scrimId: string,
        player: ScrimPlayer,
    ): Promise<void> {
        let [players] = await this.redisService.getJson<ScrimPlayer[][]>(
            `${this.prefix}${scrimId}`,
            "$.players",
        );
    ...
    async updateScrimUnlockedStatus(
        scrimId: string,
        status: ScrimStatus,
    ): Promise<void> {
        await this.redisService.setJsonField(
    ...
    async getValidated(scrimId: string): Promise<boolean> {
        ...
        const submissionId = await this.redisService.getJson(
            `${this.prefix}${scrimId}`,
            "$.submissionId",
        );
