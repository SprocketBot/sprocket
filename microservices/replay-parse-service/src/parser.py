...
BALLCHASING_API = ballchasing.Api(
    BALLCHASING_TOKEN, sleep_time_on_rate_limit=0, print_on_rate_limit=False)
...
if PARSER != "carball" and PARSER != "ballchasing":
    raise Exception(
        f"Unknown parser {PARSER}. Please specify either 'carball' or 'ballchasing'.")
...
def _parse_ballchasing(path: str, on_progress: Callable[[str], None] = None) -> dict:
    ...
    with open(path, "rb") as replay_file:
        ...
        for i, delay in enumerate(DELAYS):
            ...
            if i > 0:
                logging.info(
                    f"Uploading replay {path} for parsing ({i + 1}/{MAX_RETRIES})...")
                on_progress(
                    f"Uploading replay to Ballchasing ({i + 1}/{MAX_RETRIES})...")
            ...
            try:
                ...
                if _parse_is_duplicate_replay(*e.args):
                    ...
                    raise Exception(
                        f"Unable to upload replay {ballchasing_id} after {MAX_RETRIES} retries, total delay of {sum(DELAYS)}")

                elif _parse_is_failed_replay(*e.args):
                    logging.error(
                        f"Parsing {path} with Ballchasing failed due to bad replay", err_body)
                    raise e
                else:
                    logging.error(
                        f"Parsing {path} with Ballchasing failed, retrying in {DELAYS[i+1]} seconds", err_body)
                    continue
        ...
        for i, delay in enumerate(DELAYS):
            ...
            if i > 0:
                logging.debug(
                    f"Getting replay {ballchasing_id} from Ballchasing ({i + 1}/{MAX_RETRIES})...")
                on_progress(
                    f"Getting stats from Ballchasing ({i + 1}/{MAX_RETRIES})...")
            ...
            try:
                ...
                if i == MAX_RETRIES - 1:
                    raise Exception(
                        f"Unable to parse replay {ballchasing_id} after {MAX_RETRIES} retries, total delay of {sum(DELAYS)}")

                if _is_rate_limit(*e.args):
                    logging.debug(
                        f"Rate limited, retrying in {DELAYS[i+1]} seconds")
                    continue
                else:
                    logging.error(
                        f"Getting replay {ballchasing_id} from Ballchasing failed, retrying in {DELAYS[i+1]} seconds", err_body)
                    continue
            ...
            if _get_is_ok_replay(body):
                ...
                logging.debug(
                    f"Replay {ballchasing_id} still pending, retrying in {DELAYS[i+1]} seconds")
                continue
            elif _get_is_failed_replay(body):
                raise Exception(
                    f"Got replay that failed parsing from ballchasing {ballchasing_id}")
