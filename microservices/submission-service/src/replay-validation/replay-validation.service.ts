...
export class ReplayValidationService {
   ...
   */
    ...
    private async validateLFSSubmission(submission: LFSReplaySubmission): Promise<ValidationResult> {
        ...
        const gameResult = await this.coreService.send(CoreEndpoint.GetGameByGameMode, {
        ...
        const validationError = this.generatePlayerValidationError(
            playersResponse.data,
            uniqueBallchasingPlayerIds,
        );
        ...
        const submissionUserIds: ScrimPlayer[][][] = stats.map(s => [s.blue, s.orange].map(t => t.players.map(sp => {
            const user = players.find(p => p.request.platform === sp.id.platform.toUpperCase()
              && p.request.platformId === sp.id.id)!;
            const scrimPlayer = {
                id: user.data.userId,
                name: sp.name,
                joinedAt: now,
        ...
        const requiredUniquePlayers: number
      = (gameResult.data.mode?.teamCount ?? 0) * (gameResult.data.mode?.teamSize ?? 0);
    ...
    private async validateScrimSubmission(submission: ScrimReplaySubmission): Promise<ValidationResult> {
        ...
        // Get platformIds from stats
        // Don't send the same request for multiple players
        const uniqueBallchasingPlayerIds = Array.from(new Set(stats.flatMap((s, idx) => {
            this.logger.debug(`Game ${idx} - Blue team: ${JSON.stringify(s.blue.players.map(p => ({
                name: p.name, platform: p.id.platform, id: p.id.id,
            })))}`);
            this.logger.debug(`Game ${idx} - Orange team: ${JSON.stringify(s.orange.players.map(p => ({
                name: p.name, platform: p.id.platform, id: p.id.id,
            })))}`);
            return [s.blue, s.orange].flatMap(t => t.players.flatMap(p => ({
                name: p.name,
                platform: p.id.platform.toUpperCase(),
                id: p.id.id,
            })));
        ...
        const submissionUserIds = stats.map((s, gameIdx) => {
            const gameUserIds = [s.blue, s.orange].map((t, teamIdx) => {
                const teamUserIds = t.players.map(({id}) => {
                    this.logger.debug(`Game ${gameIdx}, Team ${teamIdx}: Looking for platform=${id.platform.toUpperCase()}, platformId=${id.id}`);
                    const user = players.find(p => p.request.platform === id.platform.toUpperCase() && p.request.platformId === id.id);
                    if (!user) {
                        this.logger.error(`Could not find user for platform=${id.platform.toUpperCase()}, platformId=${id.id}`);
    ...
    private generatePlayerValidationError(
        ...
        for (const player of playersResponseData) {
            ...
            if (!player.success) {
                if (
                    !playerErrors.some(e => e.platform === reqInput.platform && e.id === reqInput.platformId)
                ) {
                    playerErrors.push({
                        name: name,
    ...
    private async validateMatchSubmission(submission: MatchReplaySubmission): Promise<ValidationResult> {
        ...
        const uniqueBallchasingPlayerIds = Array.from(new Set(submission.items.flatMap(s => {
            const data = this.normalizeReplayData(s);
            return [data.blue, data.orange].flatMap(t => t.players.flatMap(p => ({
                name: p.name,
                platform: p.id.platform.toUpperCase(),
                id: p.id.id,
            })));
        ...
        for (const item of submission.items) {
            ...
            try {
                const bluePlayers = blueBcPlayers.map(bcp => players.find(p => p.request.platform === bcp.id.platform.toUpperCase()
                && p.request.platformId === bcp.id.id)!.data);
                const orangePlayers = orangeBcPlayers.map(bcp => players.find(p => p.request.platform === bcp.id.platform.toUpperCase()
                && p.request.platformId === bcp.id.id)!.data);
        ...
        const validateErrors = this.validateTeams(
            uniquePlayersLimit,
            perFilePlayerLimit,
