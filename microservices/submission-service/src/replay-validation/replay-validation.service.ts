...
export class ReplayValidationService {
  ...
  private async validateLFSSubmission(submission: LFSReplaySubmission): Promise<ValidationResult> {
    ...
    const submissionUserIds: ScrimPlayer[][][] = stats.map(s =>
      [s.blue, s.orange].map(t =>
        t.players.map(sp => {
          const user = players.find(
            p =>
              p.request.platform === sp.id.platform.toUpperCase() &&
  ...
  private async validateScrimSubmission(
    ...
    try {
      ...
      this.logger.error(
        `Could not get stats from minio, scrim=${scrim.id} submissionId=${scrim.submissionId}. Error: ${e}`,
      );
      // return {
      //     valid: false,
      //     errors: [ {error: "The submission is missing stats. Please contact support."} ],
    ...
    if (stats.length !== gameCount) {
      this.logger.error(
        `Unable to validate submission with missing stats, scrim=${scrim.id} submissionId=${scrim.submissionId}`,
      );
      return {
        valid: false,
        errors: [{ error: 'The submission is missing stats. Please contact support.' }],
  ...
  private generatePlayerValidationError(
    ...
    for (const player of playersResponseData) {
      ...
      const original = uniqueBallchasingPlayerIds.find(
        bc => bc.platform === reqInput.platform && bc.id === reqInput.platformId,
      );
