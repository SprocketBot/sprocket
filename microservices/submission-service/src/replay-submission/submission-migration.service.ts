...
export class SubmissionMigrationService {
    ...
    async migrateSubmissions(): Promise<void> {
        ...
        for (const key of submissionKeys) {
            try {
                const submission = await this.redisService.getJson<ReplaySubmission | EnhancedReplaySubmission>(key);
                ...
                this.logger.error(`Failed to migrate submission at key ${key}: ${error.message}`, error.stack);
                errorCount++;
            }
        }
    ...
    private isLegacy(submission: ReplaySubmission | EnhancedReplaySubmission): submission is ReplaySubmission {
        return !("franchiseValidation" in submission) || (submission.ratifiers.length > 0 && typeof submission.ratifiers[0] === "number");
    }
    ...
    private async getFranchiseInfo(playerId: number): Promise<FranchiseInfo | undefined> {
        const result = await this.coreService.send(CoreEndpoint.GetPlayerFranchises, { memberId: playerId });
