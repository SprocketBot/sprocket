...
import type { EnhancedReplaySubmission, FranchiseInfo, RatifierInfo } from '@sprocketbot/common';
...
export class ReplaySubmissionCrudService {
  ...
  async updateItemProgress(
    ...
    if (!item)
      throw new Error(`Task with id ${progress.taskId} not found for submission ${submissionId}`);
