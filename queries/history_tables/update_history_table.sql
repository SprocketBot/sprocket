...
BEGIN
    ...
    IF tg_op != 'DELETE' AND new.id IS NOT NULL THEN
        ...
        WITH all_keys    AS (SELECT column_name AS k
             ...
             select_cols AS (SELECT STRING_AGG(DISTINCT CONCAT('', '(n.x).', CONCAT('"',k,'"')), ', ') AS cs FROM all_keys),
             insert_cols AS (SELECT CONCAT(STRING_AGG(DISTINCT CONCAT('"',k,'"'), ', '), ', applicable_period') AS ci FROM all_keys)
