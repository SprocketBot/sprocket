...
export function normalizeLeagueAbbrev(abbrev: string): League {
    ...
    if (!league) {
        throw new BadRequestException(`Unknown league abbreviation "${abbrev.trim()}". Expected one of: ${Object.keys(LEAGUE_BY_ABBREV).join(", ")}`);
    }
    return league;
}
   ...
   */
    async importRow(row: NcpTeamRoleInput, actor: string): Promise<number> {
        ...
        if (seriesLeague !== league) {
            throw new BadRequestException(`Series ${row.matchId} league is "${series.league}" but row has ${league} (${row.leagueAbbrev})`);
        }

        const letters = normalizeSlotLetters(row.slotsUsed);
        if (letters.length === 0) {
            throw new BadRequestException(`No non-empty slots for series ${row.matchId}, team "${teamName}"`);
        }
        ...
        if (team.skillGroup.id !== match.skillGroupId) {
            throw new BadRequestException(`Resolved team id ${team.id} skill group ${team.skillGroup.id} does not match `
        ...
        for (const letter of letters) {
            ...
            if (!rosterRole) {
                this.logger.warn(`Skipping sprocket.roster_role_usage: no roster_role for code ${role}, skillGroupId `
            ...
            if (!rosterSlot?.player) {
                this.logger.warn(`Skipping sprocket.roster_role_usage: no roster_slot with player for team id ${team.id}, `
            + `roster role ${rosterRole.code} (match ${row.matchId}, team "${teamName}", slot ${letter})`);
                continue;
            }

            sprocketToSave.push(this.rosterRoleUsageRepo.create({
                match,
                team,
                rosterRole,
        ...
        for (const usage of sprocketToSave) {
            try {
                ...
                this.logger.warn(`Failed to save sprocket.roster_role_usage for match ${usage.match.id}, team ${usage.team.id}, `
    ...
    private async resolveFranchiseByTeamName(teamName: string): Promise<{id: number;}> {
        ...
        if (franchiseIds.length === 0) {
            throw new BadRequestException(`No franchise found with profile title or code matching "${pattern}" (case-insensitive)`);
        }
        if (franchiseIds.length > 1) {
            throw new BadRequestException(`Team name "${pattern}" matches multiple franchises (ids: ${franchiseIds.join(", ")})`);
        }
    ...
    private async resolveTeamForFranchiseAndMatchLeague(
        ...
        if (!team) {
            throw new BadRequestException(`No sprocket.team for franchise id ${franchiseId}, skill group ordinal ${skillGroupOrdinal}, `
