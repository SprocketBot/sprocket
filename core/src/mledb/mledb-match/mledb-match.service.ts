...
export class MledbMatchService {
  ...
  async getMleMatchInfoAndStakeholders(
    ...
    if (!mledbMatch.fixture)
      throw new Error(`mledb match does not have a fixture matchId=${mledbMatch.id}`);
  ...
  async markSeriesNcp(seriesId: number, isNcp: boolean, winningTeamName?: string): Promise<string> {
    this.logger.debug(
      `Begin markSeriesNcp: seriesId=${seriesId}, isNcp=${isNcp}, winningTeam=${winningTeamName}`,
    );
  ...
  async markReplaysNcp(
    ...
    this.logger.debug(
      `Begin markReplaysNcp: replayIds=${replayIds}, isNcp=${isNcp}, winningTeam=${winningTeam}`,
    );
    ...
    for (const replay of replays) {
      ...
      if (isNcp) {
        ...
        if (!replay.isDummy) {
          ...
          newWinningColor =
            winningTeam?.name === replay.winningTeamName ? replay.winningColor : null;
        } else {
        ...
        // Set winningColor depending on previous/new winningTeam
        newWinningColor = winningTeam.name === replay.winningTeamName ? replay.winningColor : null;
      }
    ...
    return `\`${
      ...
      winningTeam ? `\`winningTeam=${winningTeam.name}\`` : ''
    }.`;
  }
}
