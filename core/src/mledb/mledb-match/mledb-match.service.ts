        ...
        if (winningTeam) {
            for (const replay of replays) {
                ...
                if (!teamsInReplay.includes(winningTeam.name)) {
                    this.logger.warn(`The team \`${winningTeam.name}\` did not play in replay with id \`${
                        replay.id
                    }\` (${teamsInReplay
                        .map(t => t)
                        .join(" v. ")}), and therefore cannot be marked as the winner of this NCP. Cancelling process with no action taken.`);
