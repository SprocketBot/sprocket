    ...
    async finalizeLFS(
        ...
        try {
            ...
            if (scrim.settings.competitive) {
                ...
                const validEligibilities = eligibilities.filter(e => e !== null);
    ...
    async saveMatchDependents(
        ...
        const replays = submission.items.map<{
            ...
            = "error" in parseResult
                ...
                : "Unknown validation error";
                    throw new Error(`${i.originalFilename} does not contain expected carball values. ${errorDetails}`);
                }
            ...
            = "error" in parseResult
                ? JSON.stringify(parseResult.error.issues)
                : "Unknown validation error";
                    throw new Error(`${i.originalFilename} does not contain expected ballchasing values. ${errorDetails}`);
                }
      ...
      */
         ...
         */
            ...
            const bluePlayers = await Promise.all(blue.map(async bp => this._createPlayerStatLine(
                bp.rawPlayer,
                bp.player,
                replay.orange,
                blueTeamStats,
            ...
            const orangePlayers = await Promise.all(orange.map(async op => this._createPlayerStatLine(
                op.rawPlayer,
                op.player,
                replay.orange,
                orangeTeamStats,
