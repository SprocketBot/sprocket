...
export class ReplayParseService {
  ...
  async parseReplays(
    ...
    const member = await this.memberService.getMemberByUserIdAndOrganization(
      userId,
      organizationId,
    );
    ...
    if (!canSubmitReponse.data.canSubmit)
      throw new GraphQLError((canSubmitReponse.data as { reason: string }).reason);
  ...
  async ratifySubmission(
    ...
    if (!canRatifyReponse.data.canRatify)
      throw new GraphQLError((canRatifyReponse.data as { reason: string }).reason);
