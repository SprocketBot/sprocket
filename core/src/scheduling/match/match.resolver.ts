...
export class MatchResolver {
    ...
    // the work in its transaction.
        try {
            ...
            await this.mledbMatchService.markSeriesNcp(
                bridgeObject.seriesId,
                isNcp,
            ...
            this.logger.error(`Failed to mark series ${matchId} NCP. Got error ${e instanceof Error ? e.message : JSON.stringify(e)}`);
            throw e;
        }
    }
    ...
    @Args("winningTeamId", {nullable: true}) winningTeamId: number,
        ...
        try {
            ...
            this.logger.error(`Failed to mark replays ${roundIds} NCP. Got error ${e instanceof Error ? e.message : JSON.stringify(e)}`);
            throw e;
        }
    }
