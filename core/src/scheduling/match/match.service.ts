...
export class MatchService {
    ...
    async createMatch(isDummy?: boolean, invalidationId?: number): Promise<Match> {
        ...
        if (invalidationId) invalidation = await this.invalidationRepository.findOneOrFail({
    ...
    async createMatchWithMatchParent(
        skill_group: GameSkillGroup,
        mode: string,
        isDummy?: boolean,
        invalidationId?: number,
    ): Promise<[Match, MatchParent]> {
        let invalidation: Invalidation | undefined;
        if (invalidationId) invalidation = await this.invalidationRepository.findOneOrFail({
            where: {id: invalidationId},
        });
    ...
    async translatePayload(matchId: number, isScrim: boolean): Promise<CalculateEloForMatchInput> {
        ...
        for (const round of match.rounds) {
            ...
            orangeStats.forEach(stat => {
                if (!stat.success) {
                    errors.push(new Error(JSON.stringify(stat)));
                } else {
