...
import { addDays, isAfter, isBefore, set, startOfWeek, subDays } from 'date-fns';
    ...
    if (currentPoints < 30) {
      ...
      for (let i = 0; i < allPoints.length; i++) {
        ...
        const pointsInWindow = allPoints
          .filter(
            p => !isBefore(p.createdAt, windowStart) && !isAfter(p.createdAt, entry.createdAt),
          )
          .reduce((acc, curr) => acc + curr.points, 0);
        ...
        if (pointsInWindow >= 30) {
          ...
          for (const expirationDate of potentialExpirations) {
            const expiredEntry = allPoints.find(
              p => addDays(p.createdAt, 30).getTime() === expirationDate.getTime(),
            );
            if (expiredEntry) {
