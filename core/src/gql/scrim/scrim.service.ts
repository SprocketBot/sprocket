...
export class ScrimService {
	...
	static filterPendingScrims({ payload: scrim }: { payload: Scrim }) {
		return scrim.participants.length === 0 || scrim.state === ScrimState.PENDING;
	}
	...
	static resolveScrim(nullComplete: boolean = false) {
		return (payload: SprocketEvent<Scrim>) => {
			if (
				nullComplete &&
				[ScrimState.CANCELLED, ScrimState.COMPLETE].includes(payload.payload.state)
			) {
				return null;
			}
	...
	static filterCurrentScrim(
		...
		if (!ctx.req.user) {
			this.logger.warn(`User not found in request, AuthorizeGuard malfunction?`);
			return false;
		}
	...
	async getPendingScrims(query: ListScrimsPayload, user: User, asAdmin: boolean) {
		if (!asAdmin) {
			...
			for (const player of await dbUser.players) {
				...
				if (query.skillGroupid && player.skillGroup.id !== query.skillGroupid) continue;
	...
	async createScrim(user: User, payload: Partial<CreateScrimPayload>) {
		const player = await this.playerRepo.findOne({
			where: { id: user.id, game: { id: payload.gameId } }
		});
