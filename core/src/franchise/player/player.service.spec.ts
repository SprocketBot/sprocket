...
describe('PlayerService', () => {
  ...
  beforeEach(async () => {
    ...
    memberProfileRepository = module.get<Repository<MemberProfile>>(
      getRepositoryToken(MemberProfile),
    );
    organizationRepository = module.get<Repository<Organization>>(getRepositoryToken(Organization));
    userAuthRepository = module.get<Repository<UserAuthenticationAccount>>(
      getRepositoryToken(UserAuthenticationAccount),
    );
    ...
    mlePlayerAccountRepository = module.get<Repository<MLE_PlayerAccount>>(
      getRepositoryToken(MLE_PlayerAccount),
    );
  ...
  describe('getPlayer', () => {
    ...
    it('should throw error when player not found', async () => {
      ...
      (playerRepository.findOneOrFail as jest.Mock).mockRejectedValue(
  ...
  describe('createPlayer', () => {
    ...
    it('should handle errors during player creation', async () => {
      ...
      await expect(service.createPlayer(memberId, skillGroupId, salary)).rejects.toThrow(
        'Database error',
      );
    });
  });
  ...
  describe('updatePlayerStanding', () => {
    it('should update player salary and skill group', async () => {
      ...
      expect(playerRepository.merge).toHaveBeenCalledWith(mockPlayer, {
        salary,
        skillGroup: mockSkillGroup,
      });
  ...
  describe('swapDiscordAccounts', () => {
    it('should update discord account in MLE and Sprocket', async () => {
      ...
      expect(mlePlayerRepository.findOneOrFail).toHaveBeenCalledWith({
