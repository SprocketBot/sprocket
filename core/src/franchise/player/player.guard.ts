...
export abstract class PlayerGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        const {gameId, organizationId} = await this.getGameAndOrganization(
            ctx,
            payload,
        );
        const player = await this.playerService
            .getPlayerByOrganizationAndGame(
                payload.userId,
                organizationId,
                gameId,
            )
        ...
        if (!player)
            throw new GraphQLError(
                `User is not a player for organization=${organizationId} and game=${gameId}`,
            );
