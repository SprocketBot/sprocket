...
export class PlayerResolver {
  ...
  async franchiseName(@Root() player: Player): Promise<string> {
    ...
    const franchiseResult = await this.franchiseService.getPlayerFranchisesByMemberId(
      player.member.id,
    );
  ...
  async changePlayerSkillGroup(
    ...
    try {
      this.logger.debug(
        `Changing skill group for player ID ${playerId} to skill group ID ${skillGroupId} with salary ${salary}`,
      );
      ...
      this.logger.debug(
        `Moved player ID ${playerId} to league with skill group ID ${skillGroupId} and salary ${salary}`,
      );
  ...
  async intakeUserBulk(
    ...
    for (const csv of csvs) {
      ...
      const parsed = parseAndValidateCsv(csv, IntakeUserBulkSchema);
      if (parsed.errors.length > 0) {
        this.logger.error(
          `Errors encountered during CSV parsing: ${parsed.errors.length} errors found.`,
        );
        for (const error of parsed.errors) {
          ...
          errors.push(
            `Row ${error.row}, Field: ${error.field || 'N/A'}, Value: ${
              error.value || 'N/A'
            }, Message: ${error.message}`,
          );
        }
    ...
    for (const user of users) {
      try {
        ...
        const result = await this.playerService.intakeUser(user.name, user.discordId, [
          {
        ...
        this.logger.error(`Failed to intake user
