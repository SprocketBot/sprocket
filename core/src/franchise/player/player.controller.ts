...
export class PlayerController {
  ...
  async acceptRankdown(@Param('token') token: string): Promise<string> {
    try {
      ...
      if (player.skillGroup.id === payload.skillGroupId)
        throw new Error('You are already in this skill group');
      ...
      if (e instanceof Error) {
        if (e.name === 'TokenExpiredError')
          throw new HttpException('Rankdown request expired', 400);
  ...
  async getPlayersByPlatformIds(
    ...
    if (allResults.every(r => r.status === 'fulfilled')) {
      ...
      this.logger.verbose(
        `Successfully fetched players by platform ids: ${JSON.stringify(result)}`,
      );
      return result;
    }
