...
const entities = // @ts-expect-error bun supports top-level await
	...
	(await Promise.all(entityModules)).flatMap<Function>((mod) => Object.values(mod));
    ...
    `);
		...
		switch (process.argv[2]) {
			...
			case 'generate':
				...
				const migrationPath = path.join(__dirname, '..', 'migrations', `${stamp}-${title}.ts`);
				...
				const toQueryText = (q: Query) => {
					...
					return ['        await queryRunner.query(`', escaped, '`', params, ');'].join('');
				};
...
${downs.join('\n')}
        ...
        `.trim();
				...
				break;
			case 'test':
				console.log(datasource.entityMetadatas.length ? 'Working!' : 'Entity Metadata Missing!!');
				break;
			default:
