...
export class RbacGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // RbacService.checkPermission(userId, action, resource)
    ...
    // Token scopes are strings like "resource:action" or "*"
    if (user && (user as any).apiToken) {
      ...
      if (req && req.apiToken) {
        ...
        const hasScope =
          tokenScopes.includes('*') || tokenScopes.includes(requiredScope);

        if (!hasScope) {
          this.logger.warn(
            `API Token ${req.apiToken.id} denied access to ${requiredScope}`,
          );
          return false;
        }
      }
    ...
    return this.rbacService.checkPermission(user.id, action, resource);
  }
}
