  ...
  async seed(em: EntityManager): Promise<void> {
    ...
    const roles = [
      ...
      },
      {
        ...
        hierarchy: 1,
        description: 'Manages team roster and schedule',
      },
      {
        name: 'general_manager',
        displayName: 'General Manager',
        hierarchy: 2,
        description: 'Manages club (multiple teams)',
      },
      {
        name: 'franchise_manager',
        displayName: 'Franchise Manager',
        hierarchy: 3,
        description: 'Manages franchise (multiple clubs)',
        isRestricted: true,
      },
      {
        name: 'league_ops',
        displayName: 'League Ops',
        hierarchy: 4,
        description: 'League operations staff',
        isRestricted: true,
      },
      {
        name: 'admin',
    ...
    for (const r of roles) {
      ...
      if (!role) {
        ...
        role = await this.rbacService.createRole(
          r.name,
          r.displayName,
          r.description,
          r.hierarchy,
    ...
    // Define MVP Policies
    const policies = [
      { role: 'player', action: 'read', resource: 'profile' },
      { role: 'player', action: 'write', resource: 'profile' },
      { role: 'captain', action: 'read', resource: 'roster' },
      { role: 'captain', action: 'manage', resource: 'roster' },
      { role: 'admin', action: '*', resource: '*' },
    ];

    for (const p of policies) {
      await this.rbacService.addPermissionToRole(p.role, p.action, p.resource);
    }
