...
export class GameFeatureResolver {
    ...
    async getFeatureEnabled(
        ...
        if (!user.currentOrganizationId)
            throw new GraphQLError(`User is not related to an organization`);
        return this.gameFeatureService.featureIsEnabled(
            code,
            gameId,
    ...
    async enableFeature(
        ...
        if (!user.currentOrganizationId)
            throw new GraphQLError(`User is not related to an organization`);
        return this.gameFeatureService.enableFeature(
            code,
            gameId,
            user.currentOrganizationId,
        );
    ...
    async disableFeature(
        ...
        if (!user.currentOrganizationId)
            throw new GraphQLError(`User is not related to an organization`);
        return this.gameFeatureService.disableFeature(
            code,
            gameId,
            user.currentOrganizationId,
        );
