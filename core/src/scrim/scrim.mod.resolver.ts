...
import { CreateLFSScrimInput, CreateScrimInput, Scrim, ScrimEvent } from './types';
...
export class ScrimModuleResolver {
  ...
  async getAllScrims(
    ...
    if (!user.currentOrganizationId)
      throw new GraphQLError('Player is not connected to an organization');
  ...
  async getLFSScrims(@CurrentUser() user: UserPayload): Promise<Scrim[]> {
    ...
    const scrims = await this.scrimService.getAllScrims();
    return scrims.filter(
      s => s.organizationId === user.currentOrganizationId && s.settings.lfs,
    ) as Scrim[];
  }
  ...
  @Subscription(() => Scrim, {
    async filter(
      ...
      const teams = payload.followLFSScrims.players.map(async p => {
        ...
        const mle_player = await this.mlePlayerService.getMlePlayerBySprocketUser(
          other_player.member.userId,
        );
