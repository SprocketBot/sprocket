...
gum confirm "Build docker container?" && {
    set -e
    # BASE_URL=$(gum input --header "What URL are you deploying this image to?")
    # TODO: Set correct environment variables during the build
...
gum confirm "Execute database migrations?" && {
    ...
    echo "{ \"username\": \"$PG_USER\", \"host\": \"$PG_HOST\"  }"
    ...
    gum confirm "Proceed with migration?" && { 
        # This is a stupid hack because bun and node are treating this import differently
        sed -i -e "s/import SteamStrategy from 'passport-steam';/import * as SteamStrategy from 'passport-steam';/"
        ...
        docker run \
            ...
            -e BASE_URL=$BASE_URL \
            oven/bun:alpine \
            bun --preload reflect-metadata -b /app/core/datasource.ts up

            # This is a stupid hack because bun and node are treating this import differently
            sed -i -e "s/import * as SteamStrategy from 'passport-steam';/import SteamStrategy from 'passport-steam';/"
...
SELECTED_ACTION=$(gum choose "Connect to Database" "Deploy Flow")
