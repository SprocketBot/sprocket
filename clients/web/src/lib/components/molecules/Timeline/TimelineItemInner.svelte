...
<li class="flex flex-row gap-4 items-center mb-2 last:mb-0" on:click>
    {#if icon}
        <div class="flex-shrink-0 w-7 h-7 -ml-3.5 bg-primary rounded-full ring-4 ring-gray-700 flex items-center justify-center">
            <Icon class="w-3/4 h-3/4 text-gray-700" src={icon} theme="solid" />
        </div>
    {:else}
        <div class="flex-shrink-0 w-4 h-4 -ml-2 bg-gray-500 rounded-full ring-4 ring-gray-700 flex items-center justify-center" />
    {/if}
