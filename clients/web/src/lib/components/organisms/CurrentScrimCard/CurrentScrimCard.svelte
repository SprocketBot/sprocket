...
{#if $currentScrim}
    ...
    {:else}
        <!-- Full View -->
        <div
            class="text-gray-50 bg-gray-700 w-full p-2 rounded text-sm flex justify-center items-center flex-col gap-2"
        >
            <div class="px-4 text-left w-full">
                ...
                <p class="text-sm font-medium text-primary">
                    {$currentScrim.settings.competitive ? "Competitive" : "Casual"}
                    {$currentScrim.gameMode.description}
                </p>
                <p class="text-sm font-medium text-primary">
                    {startCase($currentScrim.settings.mode.toLowerCase())} Format
                </p>
            ...
            <div class="flex h-4 items-center text-center w-full">
                ...
                <ProgressBar
                    ...
                    variant="primary"
                    size="fill"
                    progress={($currentScrim.playerCount / $currentScrim.maxPlayers) * 100}
                />
