...
<script lang="ts">
    ...
    import type {CurrentScrim, Submission, SubmissionRejection} from "$lib/api";
    ...
    $: lastRejection =
        submission?.rejections[submission?.rejections.length - 1];
</script>
    ...
    {#if lastRejection}
        <div class="flex flex-col gap-2 mb-20">
            <div class="alert alert-error justify-start gap-6">
                ...
                <div
                    class="flex flex-col gap-1 items-start justify-start text-white"
                >
                    <span class="font-bold"
                        >{lastRejection.playerName} rejected the uploaded replays</span
                    >
                    <span>{lastRejection.reason}</span>
                    <span class="mt-2"
                        >Please upload the correct replays, or contact support
                        if you think this is a mistake.</span
                    >
                </div>
            </div>
    ...
    <div class="lobby">
        <div>
            ...
            <dl>
                ...
                <dt>Game Mode</dt>
                <dd>
                    {scrim.settings.competitive ? "Competitive" : "Casual"}
                    {screamingSnakeToHuman(scrim.settings.mode)}
                    {scrim.gameMode.description}
                </dd>
                <dt>Name</dt>
