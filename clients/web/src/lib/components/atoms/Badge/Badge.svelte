...
{#if !dismissed}
    <span class="size-{size} color-{color} inline-flex mr-2 rounded items-center gap-1">
        {#if icon}
            <span class="icon">
        ...
        <slot />
        {#if dismissible}
            <button on:click={() => (dismissed = true)} type="button" class="ml-auto -mx-1.5 rounded-lg focus:ring-2">
