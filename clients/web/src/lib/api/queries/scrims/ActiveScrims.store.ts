...
> {
  ...
  protected handleGqlMessage = (
    message: OperationResult<ActiveScrimsSubscriptionValue, ActiveScrimsStoreSubscriptionVariables>,
  ): void => {
    if (message?.data?.activeScrims) {
      ...
      switch (message.data.activeScrims.event) {
        ...
        case EventTopic.ScrimDestroyed:
          this.currentValue.data.activeScrims = this.currentValue.data.activeScrims.filter(
            s => s.id !== scrim.id,
          );
          break;
        default:
