...
const enableGameFeatureMutation = async (
  ...
  const r = await client
    .mutation<{}, Omit<SetGameFeatureVariables, 'value'>>(enableGameFeatureMutationString, vars)
    .toPromise();
  if (r.data) {
    return;
  }
...
const disableGameFeatureMutation = async (
  ...
  const r = await client
    .mutation<{}, Omit<SetGameFeatureVariables, 'value'>>(disableGameFeatureMutationString, vars)
    .toPromise();
  if (r.data) {
    return;
  }
