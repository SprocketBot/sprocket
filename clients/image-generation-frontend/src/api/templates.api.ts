...
export async function getTemplate(id: string): Promise<ImageType> {
    // if (!browser) return ;
    return handleApiResponse<ImageType>(
        await fetch(`/api/imageTypes/${encodeURI(id)}`),
    );
}
...
export async function uploadTemplate(
    ...
    const res = await handleApiResponse<string>(
        await fetch(`/api/images/${reportType}/${reportName}`, {
            method: "POST",
        }),
    );
...
export async function downloadImage(
    ...
    const res = await handleApiResponse<any>(
        await fetch(`/api/images/${reportType}/${filename}`),
    );
