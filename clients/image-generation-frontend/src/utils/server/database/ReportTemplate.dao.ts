...
export type ReportTemplateGetAll = Array<
    Omit<ReportTemplate, "templateStructure">
>;
...
class ReportTemplateDAO {
    ...
    async getAll(): Promise<ReportTemplateGetAll> {
        const results = await knexClient
            .select("reportCode", "displayName", "description")
            .from("sprocket.image_template");
    ...
    async runReport(
        ...
        if (!result?.data)
            throw new Error(
                "Missing required property 'data' from report results",
            );
        return result.data;
    }
