...
<section class="controls">
    <header>
        <h3
            ...
            class={$workstate === WorkState.Linking ? "selected" : ""}
        >
            Assign Values
        </h3>
        <h3
            on:click={() => {
                $workstate = WorkState.Fonts;
            }}
            class={$workstate === WorkState.Fonts ? "selected" : ""}
        >
            Upload Font Files
        </h3>
        <h3
            on:click={() => {
                $workstate = WorkState.Saving;
            }}
            class={$workstate === WorkState.Saving ? "selected" : ""}
        >
            Save Image
        </h3>
    </header>
    <section class="container">
        {#if !$previewEl}
            downloading Image
        {:else if $workstate === WorkState.Linking}
            <LinkControls />
