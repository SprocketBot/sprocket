...
export class DiscordSyncMarshal extends Marshal {
    ...
    async guildMemberUpdated([
        ,
        newMember,
    ]: ClientEvents[ClientEvent.guildMemberUpdate
    ]): Promise<void> {
        ...
        for (const alternateGuild of alternateGuildsWithMember) {
            ...
            if (primaryGuildMember.displayName !== alternateGuildMember.displayName) alternateGuildMember.setNickname(primaryGuildMember.displayName).catch(e => {
                this.logger.error(e);
            });
            ...
            const rolesToAdd = alternateGuildRolesFromPrimaryGuild.filter(r => primaryGuildMember.roles.cache.some(rr => rr.name === r.name)
          && !alternateGuildMember.roles.cache.some(rr => rr.name === r.name));
            const rolesToRemove = alternateGuildRolesFromPrimaryGuild.filter(r => !primaryGuildMember.roles.cache.some(rr => rr.name === r.name)
          && alternateGuildMember.roles.cache.some(rr => rr.name === r.name));
